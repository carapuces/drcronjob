...
type ControllerV2 struct {
	...
	drCronJobControl cjControlInterface

	jobLister       batchv1listers.JobLister
	drCronJobLister drbatchv1listers.DRCronJobLister

	jobListerSynced       cache.InformerSynced
	drCronJobListerSynced cache.InformerSynced
...
func NewControllerV2(ctx context.Context, jobInformer batchv1informers.JobInformer, drCronJobsInformer drbatchv1informers.DRCronJobInformer, kubeClient clientset.Interface, drkubeClient drclientset.Interface) (*ControllerV2, error) {
	...
	jm := &ControllerV2{
		...
		drCronJobLister: drCronJobsInformer.Lister(),

		jobListerSynced:       jobInformer.Informer().HasSynced,
		drCronJobListerSynced: drCronJobsInformer.Informer().HasSynced,
		now:                   time.Now,
	}
	...
	drCronJobsInformer.Informer().AddEventHandler(cache.ResourceEventHandlerFuncs{
...
func (jm *ControllerV2) Run(ctx context.Context, workers int) {
	...
	logger := klog.FromContext(ctx)
	logger.Info("Starting drcronjob controller v2")
	defer logger.Info("Shutting down drcronjob controller v2")

	if !cache.WaitForNamedCacheSync("cronjob", ctx.Done(), jm.jobListerSynced, jm.drCronJobListerSynced) {
		return
	}
...
func (jm *ControllerV2) processNextWorkItem(ctx context.Context) bool {
	...
	switch {
	case err != nil:
		utilruntime.HandleError(fmt.Errorf("error syncing DRCronJobController %v, requeuing: %w", key, err))
...
func (jm *ControllerV2) sync(ctx context.Context, cronJobKey string) (*time.Duration, error) {
	...
	logger := klog.FromContext(ctx)
	drcronJob, err := jm.drCronJobLister.DRCronJobs(ns).Get(name)
	switch {
	case errors.IsNotFound(err):
		// may be cronjob is deleted, don't need to requeue this key
		logger.V(4).Info("DRCronJob not found, may be it is deleted", "drcronjob", klog.KObj(drcronJob), "err", err)
		return nil, nil
	case err != nil:
		// for other transient apiserver error requeue with exponential backoff
		return nil, err
	}
	klog.Infof("DRCronJob %s/%s found", ns, name)
	jobsToBeReconciled, err := jm.getJobsToBeReconciled(drcronJob)
	if err != nil {
		return nil, err
	}
...
// of the correct Kind.
func (jm *ControllerV2) resolveControllerRef(namespace string, controllerRef *metav1.OwnerReference) *drbatchv1.DRCronJob {
	// We can't look up by UID, so look up by Name and then verify UID.
	// Don't even try to look up by Name if it's the wrong Kind.
	if controllerRef.Kind != controllerKind.Kind {
		return nil
	}
	cronJob, err := jm.drCronJobLister.DRCronJobs(namespace).Get(controllerRef.Name)
	if err != nil {
		return nil
	}
...
func (jm *ControllerV2) getJobsToBeReconciled(cronJob *drbatchv1.DRCronJob) ([]*batchv1.Job, error) {
	...
	klog.Infof("Found %d jobs to be reconciled", len(jobsToBeReconciled))
...
func (jm *ControllerV2) enqueueController(obj interface{}) {
	...
	klog.Infof("key=%s, obj=%v", key, obj)
	if err != nil {
...
func (jm *ControllerV2) updateCronJob(logger klog.Logger, old interface{}, curr interface{}) {
	oldCJ, okOld := old.(*drbatchv1.DRCronJob)
	newCJ, okNew := curr.(*drbatchv1.DRCronJob)
...
func (jm *ControllerV2) syncCronJob(
	...
	for _, j := range jobs {
		...
		if !found && !jobutil.IsJobFinished(j) {
			cjCopy, err := jm.drCronJobControl.GetDRCronJob(ctx, cronJob.Namespace, cronJob.Name)
			if err != nil {
	...
	klog.Infof("Update cronjob status: active=%d", len(cronJob.Status.Active))
	...
	if cronJob.Spec.Suspend != nil && *cronJob.Spec.Suspend {
		klog.Info("Not starting job because the cron is suspended", "cronjob", klog.KObj(cronJob))
		return nil, updateStatus, nil
	}
	...
	if err != nil {
		// this is likely a user error in defining the spec value
		// we should log the error and not reconcile this cronjob until an update to spec
		klog.Info("Unparseable schedule", "cronjob", klog.KObj(cronJob), "schedule", cronJob.Spec.Schedule, "err", err)
	...
	if err != nil {
		// this is likely a user error in defining the spec value
		// we should log the error and not reconcile this cronjob until an update to spec
		klog.Error("Invalid schedule", "cronjob", klog.KObj(cronJob), "schedule", cronJob.Spec.Schedule, "err", err)
	...
	if cronJob.Spec.ConcurrencyPolicy == drbatchv1.ForbidConcurrent && len(cronJob.Status.Active) > 0 {
	...
	if cronJob.Spec.ConcurrencyPolicy == drbatchv1.ReplaceConcurrent {
		for _, j := range cronJob.Status.Active {
	...
	klog.Infof("Get Job from template %s/%s", cronJob.Namespace, cronJob.Name)
	jobReq, err := getJobFromTemplate2(cronJob, *scheduledTime)
	if err != nil {
		klog.Error("Get Job from template failed", "cronjob", klog.KObj(cronJob))
		logger.Error(err, "Unable to make Job from template", "cronjob", klog.KObj(cronJob))
		return nil, updateStatus, err
	}
	jobResp, createErr := jm.jobControl.CreateJob(cronJob.Namespace, jobReq)
	if createErr != nil {
...
func getJobName(cj *drbatchv1.DRCronJob, scheduledTime time.Time) string {
	klog.Infof("Get Job scheduledTime: %v", scheduledTime.String())
	return fmt.Sprintf("%s-%d", cj.Name, getTimeHashInSecond(scheduledTime))
}

// cleanupFinishedJobs cleanups finished jobs created by a CronJob
// It returns a bool to indicate an update to api-server is needed
func (jm *ControllerV2) cleanupFinishedJobs(ctx context.Context, cj *drbatchv1.DRCronJob, js []*batchv1.Job) bool {
...
func (jm *ControllerV2) removeOldestJobs(ctx context.Context, cj *drbatchv1.DRCronJob, js []*batchv1.Job, maxJobs int32) bool {
	updateStatus := false
...
func deleteJob(logger klog.Logger, cj *drbatchv1.DRCronJob, job *batchv1.Job, jc jobControlInterface, recorder record.EventRecorder) bool {
...
func formatSchedule(cj *drbatchv1.DRCronJob, recorder record.EventRecorder) string {
