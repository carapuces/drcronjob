...
func TestGetJobFromTemplate2(t *testing.T) {
	...
	cj := drbatchv1.DRCronJob{
		...
		Spec: drbatchv1.DRCronJobSpec{
			Schedule:          "* * * * ?",
			ConcurrencyPolicy: drbatchv1.AllowConcurrent,
			JobTemplate: drbatchv1.JobTemplateSpec{
				ObjectMeta: metav1.ObjectMeta{
					CreationTimestamp: metav1.Time{Time: scheduledTime},
					Labels:            map[string]string{"a": "b"},
				},
				Spec: drbatchv1.JobSpec{
	...
	tests := []struct {
		...
		},
		{
			...
			cj: &drbatchv1.DRCronJob{
				...
				Status: drbatchv1.DRCronJobStatus{
